// Source: https://github.com/superwall/iOS-Backports/blob/main/Sources/SwiftUIBackports/SwiftUIBackports.swift
...
public extension Backport where Content: View {
    ...
    @ViewBuilder func zoom(sourceID: some Hashable,
    ...
    @ViewBuilder func matchedTransitionSource(id: some Hashable,
                                              in namespace: Namespace.ID) -> some View {
    ...
    @ViewBuilder func imagePlayground(_ presented: Binding<Bool>,
                                      completion: @escaping (URL?) -> Void) -> some View {
...
public extension BackportGlass {
    var toGlass: Glass {
        switch self {
        case .regular:
            return .regular
        ...
        case let .tintedAndInteractive(color, isEnabled):
...
public extension Backport where Content: View {
    ...
    @ViewBuilder func glassEffect(_ backportGlass: BackportGlass = .regular,
    ...
    @ViewBuilder func glassEffectUnion(id: (some Hashable & Sendable)?,
    ...
    @ViewBuilder func glassProminentButtonStyle() -> some View {
        if #available(iOS 26.0, macOS 26, *) {
            ...
            content.buttonStyle(GlassProminentFallbackButtonStyle())
        }
    }
    ...
    @ViewBuilder func scrollEdgeEffectStyle(_ style: BackportScrollEdgeEffectStyle?,
                                            for edges: Edge.Set) -> some View {
        if #available(iOS 26.0, macOS 26, *) {
    ...
    @ViewBuilder func scrollEdgeEffectHidden(_ hidden: Bool = true,
    ...
    @ViewBuilder func glassEffectID(_ id: (some Hashable & Sendable)?,
                                    in namespace: Namespace.ID) -> some View {
        if #available(iOS 26.0, macOS 26, *) {
    ...
    @ViewBuilder func listSectionMargins(_ edges: Edge.Set = .all, _ length: CGFloat?) -> some View {
        if #available(iOS 26.0, macOS 15, *) {
            #if os(iOS)
...
// MARK: - Fallback Button Styles

@available(iOS 14, macOS 15, *)
public struct GlassProminentFallbackButtonStyle: PrimitiveButtonStyle {
    public init() {}
